...
public class CommissionRateService : ICommissionRateService
{
    ...
    {
        var usdAmount = money.Amount;
        if (money.Currency.CurrencyType != Constants.USDCurrency)
        {
            var exchangeValue = money.Currency.FirstExchangeValues?.FirstOrDefault(exValue =>
                exValue.SecondCurrency.CurrencyType == Constants.USDCurrency);
            if (exchangeValue == null)
                return null;
            var valueToBeMultiplied = exchangeValue.UnitOfFirstValue;
            usdAmount = money.Amount * valueToBeMultiplied;
        }

        var cRate = await _commissionRateRepository.GetCRateByUSDAmountAsync(usdAmount);
        if (cRate == null)
            return null;
